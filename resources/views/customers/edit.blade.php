...
<div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-4 gap-3">
    <div>
        <h4 class="fw-bold mb-0 text-dark">{{ __('Edit Customer') }}</h4>
        <span class="text-muted small">Update customer details</span>
    </div>
    <div>
        ...
        <div class="card border-0 shadow-sm" style="border-radius: 16px;">
            <div class="card-body p-4">
                <form action="{{ route('customers.update', $customer->id) }}" method="POST">
                    ...
                    <button type="submit" class="btn btn-primary w-100 py-2 fw-semibold" style="border-radius: 8px;">{{ __('Update Customer') }}</button>
                </form>
            </div>
        </div>
    </div>
