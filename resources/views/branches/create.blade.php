...
<div class="mb-4">
    ...
    <h4 class="fw-bold mb-0 text-dark">{{ __('Add New Branch') }}</h4>
</div>
            ...
            <div class="card-body p-4">
                <form action="{{ route('branches.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control bg-light @error('name') is-invalid @enderror" value="{{ old('name') }}" required placeholder="{{ __('e.g. Mlimani City Branch') }}">
                        @error('name')
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="address" class="form-control bg-light @error('address') is-invalid @enderror" rows="2" placeholder="{{ __('Physical location of the branch') }}">{{ old('address') }}</textarea>
                        @error('address')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="d-grid mt-2">
                        <button type="submit" class="btn btn-primary py-2 fw-bold rounded-3">
                            <i class="bi bi-save me-1"></i> {{ __('Save Branch') }}
                        </button>
                    </div>
                </form>
            </div>
