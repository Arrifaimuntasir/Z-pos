        ...
        <form action="{{ route('products.update', $product) }}" method="POST">
            ...
            <div class="row g-4 mb-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="branch_id" class="form-select @error('branch_id') is-invalid @enderror" required>
                            ...
                            @foreach($branches as $branch)
                                <option value="{{ $branch->id }}" {{ old('branch_id', $productBranchId) == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                            @endforeach
                        </select>
                        ...
                        <div class="card-body">
                            ...
                            <div id="ingredients_container">
                                @foreach($product->ingredients as $index => $ing)
                                    <div class="row g-2 mb-2 ingredient-row" id="ingredient_row_{{ $index }}">
                                        <div class="col-md-7">
                                            <select name="ingredients[{{ $index }}][id]" class="form-select form-select-sm" required>
                                                <option value="">Select Ingredient...</option>
                                                @foreach($allProducts as $p)
                                                    <option value="{{ $p->id }}" {{ $ing->ingredient_id == $p->id ? 'selected' : '' }}>{{ $p->name }} (Stock: {{ $p->stock }})</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-4">
                                            <input type="number" step="0.001" name="ingredients[{{ $index }}][quantity]" class="form-control form-control-sm" value="{{ $ing->quantity }}" required>
                                        </div>
                                        <div class="col-md-1">
                                            <button type="button" class="btn btn-sm btn-outline-danger w-100" onclick="document.getElementById('ingredient_row_{{ $index }}').remove()">
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" name="expiry_date" class="form-control @error('expiry_date') is-invalid @enderror" value="{{ old('expiry_date', $product->expiry_date) }}" @if($shopCategory == 'Pharmacy / Health') required @endif>
            ...
            <div class="d-flex justify-content-end pt-3 border-top">
                ...
                <button type="submit" class="btn btn-primary px-4">{{ __('Update Product') }}</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    let ingredientIndex = {{ $product->ingredients->count() > 0 ? $product->ingredients->count() : 0 }};
