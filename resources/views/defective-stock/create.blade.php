...
<div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-4 gap-3">
    <div>
        ...
        <span class="text-muted small">{{ __('Remove defective or damaged units from stock before they are sold') }}</span>
    </div>
    <div>
    ...
    <div class="col-lg-6">
        <div class="card border-0 shadow-sm" style="border-radius: 16px;">
            <div class="card-body p-4">
                <form action="{{ route('defective-stock.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <select name="product_id" id="productSelect" class="form-select @error('product_id') is-invalid @enderror" required style="width: 100%;">
                            ...
                            @foreach($products as $product)
                                <option value="{{ $product->id }}" {{ old('product_id') == $product->id ? 'selected' : '' }} data-stock="{{ $product->current_stock }}">
                                    {{ $product->name }}{{ $product->model ? ' - ' . $product->model : '' }} ({{ __('Stock') }}: {{ $product->current_stock }})
                                </option>
                            @endforeach
                        </select>
                        @error('product_id')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="quantity" class="form-control @error('quantity') is-invalid @enderror" value="{{ old('quantity') }}" min="0.01" step="0.01" placeholder="{{ __('e.g. 2') }}" required>
                        @error('quantity')
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="reason" class="form-control" rows="3" placeholder="{{ __('e.g. Cracked screen, water damage...') }}">{{ old('reason') }}</textarea>
                    </div>

                    <button type="submit" class="btn btn-danger w-100 py-2 fw-semibold" style="border-radius: 8px;">
                        <i class="bi bi-dash-circle me-2"></i>{{ __('Remove from Stock') }}
                    </button>
                </form>
            </div>
        </div>
