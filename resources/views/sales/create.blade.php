...
<div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-4 gap-3">
    <div>
        <h4 class="fw-bold mb-0 text-dark">{{ __('Point of Sale (POS)') }}</h4>
        <span class="text-muted small">{{ __('Record new sales') }}</span>
    </div>
    <div>
        <a href="{{ route('sales.index') }}" class="btn btn-light px-4 shadow-sm" style="border-radius: 8px;">
            <i class="bi bi-clock-history me-2"></i> {{ __('Sales History') }}
        </a>
    </div>
</div>
...
<form action="{{ route('sales.store') }}" method="POST" id="posForm">
    ...
    <div class="row g-4">
                            ...
                            <tbody id="cartBody">
                                <tr id="emptyCartRow">
                                    <td colspan="5" class="text-center py-5 text-muted">
                                        ...
                                        <p class="mb-0">{{ __('Cart is empty. Select products to sell.') }}</p>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
        ...
        <div class="col-lg-4">
            <div class="card border-0 shadow-sm mb-4" style="border-radius: 16px;">
                <div class="card-body p-4">
                    <h6 class="fw-bold mb-4">{{ __('Sale Details') }}</h6>
                    
                    <div class="mb-3">
                        <label class="form-label text-muted small fw-medium">{{ __('Reference / Receipt No.') }}</label>
                        <input type="text" class="form-control bg-light border-0" value="{{ $reference_no }}" readonly>
                    </div>

                    <div class="mb-3">
                        <label class="form-label text-muted small fw-medium">{{ __('Date') }}</label>
                    ...
                    <div class="d-flex justify-content-between mb-4">
                        <h4 class="fw-bold mb-0">{{ __('Total') }}</h4>
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-bold text-dark">{{ __('Amount Paid') }} (TSh) <span class="text-danger">*</span></label>
